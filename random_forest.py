...
class RandomForest:
    ...
    num_tree: int, default=5
        The number of decision tree classifiers used
    subsample_size: float, default=None
        The proportion of the total training examples used to train each decision tree
    ...
    max_features: int, float, default=None
        ...
        if None, then it will use the sqrt function -> max_features = sqrt(n_features)
    ...
    def sample(self, X, y, random_state):
        ...
        Returns:
        sample of features: np.array, feature bootstrapped sample of target: np.array, corresponding target bootstrapped sample
        """
    ...
    def fit(self, X, y):
        """
        Trains a Random Forest classifier object, with the corresponding rules stored as attributes in the nodes in each
        decision tree.
